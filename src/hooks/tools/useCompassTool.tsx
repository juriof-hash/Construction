...
import { distance, angleBetween, describeArc, polarToCartesian } from '../../utils/mathUtils';
...
export const useCompassTool = (
  ...
  // rAF rendering state
  const [renderSweep, setRenderSweep] = useState({ startAngle: 0, totalSweep: 0 });
  const [pencilHoverAngle, setPencilHoverAngle] = useState<number | null>(null);
  const rafRef = useRef<number>(0);
  const pencilRafRef = useRef<number>(0);

  // Status indicator
  const [lastTouchCoords, setLastTouchCoords] = useState<{ x: number; y: number } | null>(null);
  ...
  const onPointerMove = useCallback((e: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (step === 'PENCIL') {
      ...
      rafRef.current = requestAnimationFrame(() => {
        setRenderSweep({ startAngle: startAngleRef.current, totalSweep: totalSweepRef.current });
      });
    }
  ...
  const onPointerUp = useCallback((e: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (step === 'SWEEP' && isStartedRef.current) {
       ...
       if (Math.abs(ts) > 0.05) { // 헛스윙(CANCEL) 방어
          dispatch({
            type: 'ADD_GEOMETRY',
            payload: {
              id: generateId(),
  ...
  const preview = (
    <g className="pointer-events-none">
      {center && <circle cx={center.x} cy={center.y} r={4} fill="#f59e0b" />}
      
      {step === 'PENCIL' && radiusPt && (
        <>
          <line x1={center!.x} y1={center!.y} x2={radiusPt.x} y2={radiusPt.y} stroke="#f59e0b" strokeWidth={1} strokeDasharray="4 4" />
          <circle cx={center!.x} cy={center!.y} r={distance(center!, radiusPt)} stroke="#f59e0b" strokeOpacity={0.2} strokeWidth={1} fill="none" />
        </>
      )}
      ...
      {step === 'SWEEP' && center && (
         <>
           ...
           <circle cx={center.x} cy={center.y} r={r} stroke="#f59e0b" strokeOpacity={0.15} strokeWidth={1} fill="none" />
           
           {/* Active drawing sweep */}
           {Math.abs(renderSweep.totalSweep) > 0 && (
             <path 
                d={describeArc(center.x, center.y, r, renderSweep.startAngle, renderSweep.totalSweep)}
                fill="none" 
                stroke="#f59e0b" 
                strokeWidth={2}
             />
           ...
           {/* Pencil indicator */}
           {(!isStartedRef.current && pencilHoverAngle !== null) && (
              <circle 
                cx={polarToCartesian(center.x, center.y, r, pencilHoverAngle).x} 
                cy={polarToCartesian(center.x, center.y, r, pencilHoverAngle).y}
                r={4} fill="#f59e0b" 
              />
           )}
         </>
      ...
      {hoverPt && !isStartedRef.current && step !== 'SWEEP' && (
        <>
          ...
          <circle cx={hoverPt.pt.x} cy={hoverPt.pt.y} r={hoverPt.snapped ? 4 : 3} fill={hoverPt.snapped ? "#eab308" : "#3b82f6"} opacity={hoverPt.snapped ? 1 : 0.5}/>
        </>
      )}
    </g>
  );
  ...
  const touchOffsetIndicator = (activeMode === 'touch' && lastTouchCoords) ? (
      <div 
