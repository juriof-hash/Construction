...
export const useLineTool = (
  ...
  const [currPt, setCurrPt] = useState<{pt: Point2D, snapped: boolean, raySnapPt?: Point2D} | null>(null);
  ...
  const calcRaySnap = useCallback((mousePt: Point2D, basePt: Point2D) => {
  ...
  const preview = (
    <g className="pointer-events-none">
      ...
      {currPt && currPt.raySnapPt && (
        <line x1={startPt!.x} y1={startPt!.y} x2={currPt.raySnapPt.x} y2={currPt.raySnapPt.y} stroke="#eab308" strokeWidth={1} strokeDasharray="2 2" />
      )}
      {currPt && (
        <>
